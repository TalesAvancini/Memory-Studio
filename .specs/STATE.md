...
- **phase:** "Phase 7b — Real API Measurement + Tuning"
- **phase-previous:** "Phase 7a — Metrics Instrumentation" (DONE 2026-08-02 via single batch (no subchapters — 7 atomic tasks). 478 root + 152 UI + 16 SDK = 646 tests. Gates green twice. POC re-run 4/4 PASS at total overhead p95 0.14-0.21ms (≪ 0.30ms Phase 7a ceiling, ≪ 10ms Phase 6b invariant). 8/8 forge tests PASS. AC-1..AC-14 all PASS. New `GET /metrics` endpoint (8th endpoint, schema_version=1) — NOT extension of `/health`. R-1 naming divergence (Phase 7a = match-pipeline hit, R-2 = provider cache hit) + R-2 formula simplification (request-weighted vs PRD token-weighted) explicitly documented in spec.md §5 and validation-phase-7a.md. Implementer commits `7e85356`, `f65fbc3`, `932d336`, `245b32c`, `4e51efd`, `11bfc6e`, `03cee68` (T-01..T-07). Post-Implementer `87c9c96` adds `low_confidence` outcome mapping. Final HEAD at `ca3b22c`. Non-blocking gaps (R-2 denominator on 200 response without `usage`, sliding-window wording vs cumulative counters, fractional p50/p99 vs integer comments, missing pino info log) carried to Phase 7b cleanup — NOT iter-2 fix-tasks)
...
- **próximo-step-concreto:** dispatch Implementer Phase 7b Batch 1 (T-01..T-06 — autonomous production wiring + acceptance scaffolding, 5-7h). Includes L-006 critical fixes: state.json thresholds consumed by runAugment (T-01), proxy forwards exact matched system + active catalog + session ID + streaming (T-02..T-03), metrics v2 + R-2 denominator edge + sliding-window/cumulative + fractional latency (T-04), acceptance evaluator + snapshot tool + smoke (T-05..T-06). After Verifier 7b.1 PASS, ORCHESTRATOR PAUSES for user-driven T-07 (≥7 days wall-clock). Then T-08 final hydration.
- **phase-7b-plan:** Planner artifacts at `.specs/features/phase-7b-acceptance-gate/{spec.md, design.md, tasks.md}` (commit `c_pending`). 8 atomic tasks (T-01..T-08) across 3 execution batches (NO subchapter split — within 8-task cap). 7b.1 = T-01..T-06 autonomous (5-7h). 7b.2 = T-07 USER-DRIVEN wall-clock (≥7×24h). 7b.3 = T-08 closure (0.5-1.5h). Carries 4 Phase 7a Verifier gaps (R-2 denominator, sliding-window, fractional latency, proxy T-14 deviation) into T-04. Deferred to v3.1+: /metrics pino info logging, POST /catalog/rebuild production wiring, test#366 port-range cleanup. Planner's L-006 critical findings: state.json NOT consumed by runAugment (defaults 0.75/1 vs configured 0.60/2), proxy hardcodes `activeCatalog: []` + session ID "proxy" + discards matched pipeline output.
- **phase-7a-status:** "Phase 7a — Metrics Instrumentation" (DONE 2026-08-02, 1 iteration, Verifier PASS at `ca3b22c`. 7 atomic tasks T-01..T-07. 478 root + 152 UI + 16 SDK = 646 tests. Gates green twice. POC re-run 4/4 PASS at total overhead p95 0.14-0.21ms. 8/8 forge tests. AC-1..AC-14 all PASS. New `GET /metrics` endpoint. R-1 naming + R-2 formula simplification divergences documented. Non-blocking gaps carried to Phase 7b. Validation: `.specs/features/phase-7a-metrics/validation-phase-7a.md`. Commits `7e85356`, `f65fbc3`, `932d336`, `245b32c`, `4e51efd`, `11bfc6e`, `03cee68`, `87c9c96`)
...
- **working-tree:** working (`loop/phase-0` em `ca3b22c`)
